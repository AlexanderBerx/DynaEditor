...
class Float3Attribute(BaseAttribute):
    def __init__(self, attr, default_value, nice_name, color=False, _min=None, _max=None):
        if not _min:
            _min = [0, 0, 0]
        if not _max:
            _max = [100, 100, 100]
    ...
    @staticmethod
    def validate_args(attr=None, nice_name=None, default_value=None, color=None, _min=None, _max=None):
